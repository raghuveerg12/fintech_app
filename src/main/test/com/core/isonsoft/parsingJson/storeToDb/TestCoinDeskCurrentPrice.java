...
public class TestCoinDeskCurrentPrice {
	...
	public void testConvertJsonDataToDb() throws IOException {
		...
		String url = InsertToStatsDownTable
				.getUrlFromCodeDataTab(session, "10");

		CoinDeskCurrentPrice.convertJsonDataToDb(
				ConvertUrlToJsonFile.readJsonFromUrl(url, "10"),
				System.currentTimeMillis(), session);

		Session session1 = HibernateUtil.getSessionFactory().openSession();
		session1.beginTransaction();
		Query query = session1
				.createQuery("from com.core.isonsoft.beans.CoinDeskCurrentPriceMainBean");
		List<CoinDeskCurrentPriceMainBean> results = query.getResultList();
