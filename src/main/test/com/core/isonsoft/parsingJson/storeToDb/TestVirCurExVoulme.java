...
public class TestVirCurExVoulme {
	...
	public void testConvertJsonDataToDb() throws IOException {
		...
		String url = InsertToStatsDownTable
				.getUrlFromCodeDataTab(session, "16");

		VirCurExVolume.convertJsonDataToDb(
				ConvertUrlToJsonFile.readJsonFromUrl(url, "16"),
				System.currentTimeMillis(), session);

		Session session1 = HibernateUtil.getSessionFactory().openSession();
		session1.beginTransaction();
		Query query = session1
				.createQuery("from com.core.isonsoft.beans.StagVirCurExVolumeBean");
